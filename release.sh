...
(
	...
	exec /usr/bin/env python2.7 - ${COMMIT} --release stable \
		-x '#(\d+)' 'Issue' 'https://github.com/mk23/nurly/issues/{item}' \
		-e configure.ac 'nurly-module, {version},' \
		-e server/httpd.py "VERSION = '{version}'" \
		-e README.md 'nurly-module-{version}.tar.gz' \
		-e README.md 'nurly-server-{version}.tar.gz' \
		"$@"
